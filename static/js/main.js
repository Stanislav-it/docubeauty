...
(function initSearchSuggest(){
  ...
  function bind(input, dd){
    ...
    const render = (data) => {
      ...
      if (cats.length) {
        html += `<div class="dd-section">Kategorie</div>`;
        cats.forEach(c => {
          const url = `/shop?category=${encodeURIComponent(c.slug)}`;
          html += `
            <a href="${url}">
              <span class="dd-img"></span>
              <span>
                <div>${escapeHtml(c.name)}</div>
                <div class="dd-meta">${c.count} produktów</div>
              </span>
      ...
      if (prods.length) {
        html += `<div class="dd-section">Produkty</div>`;
        prods.forEach(p => {
          const url = `/product/${encodeURIComponent(p.id)}`;
          const img = p.thumb ? `<img src="${p.thumb}" alt="">` : "";
          html += `
            <a href="${url}">
              <span class="dd-img">${img}</span>
              <span>
                <div>${escapeHtml(p.title)}</div>
                <div class="dd-meta">${escapeHtml(p.category)}</div>
              </span>
              <span class="dd-price">${escapeHtml(p.price || "")}</span>
            </a>`;
        });
      }

      if (!html) {
        html = `<div class="dd-empty">Brak podpowiedzi</div>`;
      }
    ...
    input.addEventListener('focus', () => {
      const q = (input.value || "").trim();
      if (q.length >= 2 && dd.innerHTML.trim()) open();
    });
